  ...
  return <div>
    ...
    {section==='current'&&<div className="grid cols-2">
      <div className="panel"><h3>Current Situation</h3>
        ...
        {ch.job?<><div className="kv"><span>Occupation</span><span>{jobTitle(ch.job)}</span></div><div className="kv"><span>Annual salary</span><span>{money(wageFor(country,ch.job,ch))}</span></div><div className="kv"><span>Time in current position</span><span>{ch.job.yearsAtRung} year(s)</span></div><div className="kv"><span>Field experience</span><span>{ch.experience?.sectors?.[ch.job.sector]||0} year(s)</span></div><button onClick={()=>{quitJob(state);refresh();}}>Quit job</button></>:<div className="muted">You do not currently hold a civilian job.</div>}
